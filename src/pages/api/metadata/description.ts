...
export const POST: APIRoute = async ({ request }) => {
    try {
            ...
            const relationsArray = await Promise.all(
                metadataRelations.map((rel: any) => {
                    ...
                    if (formData.common_name) {
                        // formData.common_names = '1' or formData.common_names = '[1,2]'
                        console.log("commonName: ", formData.common_name);
                        const commons = commonNames.map((commonName: string | number) => Number(commonName));
                        // If commons is an array, check if common_name is in commons array
                        if (Array.isArray(commons)) {
                            if (commons.includes(formData.common_name)) {
        ...
        // Get not null groups from metadata
        const groupIds = [...new Set(metadatos.map(m => m.id_group).filter(Boolean))].filter(id => id !== 0);
    ...
    try {
        ...
        const [commonRelations, categoryRelations, productTypeRelations] = await Promise.all([
            db.select()
                .from(MetadataRelations)
                .where(
                    eq(MetadataRelations.common_names, parseInt(commonNameId))),
            db.select()
                .from(MetadataRelations)
                .where(eq(MetadataRelations.categories, parseInt(categoryId))),
            db.select()
                .from(MetadataRelations)
